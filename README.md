...
Private DHIS2 tooling — a `uv` workspace containing a pure client library, a Typer CLI, a FastMCP server, a Playwright browser-automation helper, and a shared plugin runtime. DHIS2 v42+ only; async throughout.
...
Eleven top-level domains; every plugin shares a `service.py` between the CLI and MCP sides so one typed call answers both surfaces.
...
| `dhis2 maintenance` | Background tasks, cache clear, data-integrity, soft-delete cleanup |
