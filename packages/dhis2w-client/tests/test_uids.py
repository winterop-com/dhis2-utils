...
def test_uids_exported_from_top_level() -> None:
    """Top-level `from dhis2w_client import generate_uid` stays stable."""
    import dhis2w_client

    assert dhis2w_client.generate_uid is generate_uid
    assert dhis2w_client.is_valid_uid is is_valid_uid
    assert dhis2w_client.UID_LENGTH == 11
