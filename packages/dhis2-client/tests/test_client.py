...
async def test_connect_falls_back_to_nearest_lower_version() -> None:
    ...
    client = Dhis2Client("https://dhis2.example", auth=BasicAuth("a", "b"), allow_version_fallback=True)
    try:
        await client.connect()
...
async def test_get_raw_injects_auth_header_and_parses_json() -> None:
    ...
    client = Dhis2Client("https://dhis2.example", auth=BasicAuth("admin", "district"))
