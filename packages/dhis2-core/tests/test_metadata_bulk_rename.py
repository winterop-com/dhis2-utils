...
def test_rename_cli_dry_run_prints_preview(pat_profile: None) -> None:  # noqa: ARG001
    ...
    with patch("dhis2_core.plugins.metadata.service.bulk_rename_metadata", new=AsyncMock(return_value=dry)):
        result = CliRunner().invoke(
            build_app(),
...
def test_rename_cli_renders_no_match_message(pat_profile: None) -> None:  # noqa: ARG001
    ...
    with patch("dhis2_core.plugins.metadata.service.bulk_rename_metadata", new=AsyncMock(return_value=empty)):
        result = CliRunner().invoke(
            build_app(),
