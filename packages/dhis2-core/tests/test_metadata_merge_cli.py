...
def test_cli_merge_dry_run_renders_conflict_table(profiles_toml: Path) -> None:
    ...
    with patch("dhis2_core.plugins.metadata.service.merge_metadata", mock):
...
def test_cli_merge_clean_run_skips_conflict_table(profiles_toml: Path) -> None:
    ...
    with patch("dhis2_core.plugins.metadata.service.merge_metadata", AsyncMock(return_value=clean)):
        result = CliRunner().invoke(
            build_app(),
