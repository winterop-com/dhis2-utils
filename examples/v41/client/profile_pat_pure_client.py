r"""Pure dhis2w-client example (v41): build a Profile and connect via PAT, no dhis2w-core.

This example demonstrates that PAT and Basic auth library-users can run
against DHIS2 v41 with **only `dhis2w-client` installed** — no `dhis2w-core`,
...
go through `dhis2w_client.v41` to pin the v41 plugin tree.
...
Usage:
    DHIS2_URL=http://localhost:8080 DHIS2_PAT=d2pat_xxx \
        uv run python examples/v41/client/profile_pat_pure_client.py
"""
...
from dhis2w_client.v41 import Profile, open_client


async def main() -> None:
    """Connect to DHIS2 v41 with a hand-built Profile (PAT) and print whoami."""
