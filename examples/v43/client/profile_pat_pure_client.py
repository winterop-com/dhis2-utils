r"""Pure dhis2w-client example (v43): build a Profile and connect via PAT, no dhis2w-core.

This example demonstrates that PAT and Basic auth library-users can run
against DHIS2 v43 with **only `dhis2w-client` installed** — no `dhis2w-core`,
no Typer / FastMCP / SQLAlchemy / Alembic. Profile construction and
`open_client` for PAT/Basic live in `dhis2w-client` since v0.12.0. Imports
go through `dhis2w_client.v43` to pin the v43 plugin tree.
...
Usage:
    DHIS2_URL=http://localhost:8080 DHIS2_PAT=d2pat_xxx \
        uv run python examples/v43/client/profile_pat_pure_client.py
"""
...
from dhis2w_client.v43 import Profile, open_client


async def main() -> None:
    """Connect to DHIS2 v43 with a hand-built Profile (PAT) and print whoami."""
