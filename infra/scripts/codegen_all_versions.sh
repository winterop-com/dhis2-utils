...
for v in "${VERSIONS[@]}"; do
  ...
  # Also dump each version's fresh schema state into infra/dhis-$v.sql.gz
  ...
  # a usable DHIS2 v$v without pulling + booting its image from scratch.
  ...
  if [ -n "$pg_container" ]; then
    echo ">>> dumping v$v schema to infra/dhis-$v.sql.gz"
    if docker exec "$pg_container" pg_dump --no-owner --no-privileges --no-sync \
          ...
          -U dhis -d dhis | gzip -9 > "$INFRA_DIR/dhis-$v.sql.gz"; then
      size=$(wc -c < "$INFRA_DIR/dhis-$v.sql.gz")
      echo "    wrote infra/dhis-$v.sql.gz ($size bytes)"
    else
      echo "!!! pg_dump failed for v$v"
      FAILED+=("v$v:dump")
      rm -f "$INFRA_DIR/dhis-$v.sql.gz"
    fi
  else
    echo "!!! no postgres container found to dump v$v"
    FAILED+=("v$v:dump-missing-container")
  fi
