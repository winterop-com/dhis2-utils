...
7. **Pydantic for ALL data models.** DHIS2 resource models (Me, SystemInfo, DataElement, Indicator, …) live in `dhis2-client/models/` so PyPI users of the client get them. Plugin-internal view-models (reports, job state, summaries) live in the plugin's `models.py`. `Dhis2Client` returns parsed models, not raw dicts.
