...
For update orchestration (compare installed version to hub latest, install newer), see `dhis2_core.plugins.apps.service.update_all` — also exposed as the `dhis2 apps update --all` CLI verb with a `--dry-run` preview mode.
