...
5. **`/api/schemas` emitter regen-stability sweep** — sibling fix to the OAS regen-stability work just landed. `make lint --fix` strips ~370 unused imports across `generated/v{40-44}/schemas/*.py` on every run, then re-runs of `dhis2 dev codegen generate` re-add them; the noise lives in `dhis2_codegen.emit` (parallel to `dhis2_codegen.oas_emit`). Apply the same conditional-import + override-import-reset treatment, port the `test_emitter_does_not_emit_unused_imports` and `test_emitter_is_regen_stable` regression tests from `test_oas_emit.py`, then commit the cleaned-up output once.
