...
2. **Spec-class audit decision** — the hand-written accessor tree has quietly drifted away from `*Spec` builders in favour of plain kwargs on `create(...)` (every triple + tracker accessor ships this way). Three older accessors still ship `*Spec` builders: `VisualizationSpec`, `MapSpec + MapLayerSpec`, `LegendSetSpec + LegendSpec`. Decision time: keep them (document the split in `api/visualizations.md` / `api/maps.md` / `api/legend-sets.md`) or deprecate + remove (breaking change; one PR). Revisit once someone's run into the spec-vs-kwargs friction in practice.
