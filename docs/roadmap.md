...
- **Authoring sub-apps**: `options show / find / sync` for OptionSet sync; `attribute get / set / delete / find` for cross-resource AttributeValue workflows; `program-rule show / vars-for / validate-expression / where-de-is-used`; `sql-view list / show / execute / refresh / adhoc`; `viz list / show / create / clone / delete`; `dashboard list / show / add-item / remove-item`; `map list / show / create / clone / delete`; `legend-sets list / show / create / clone / delete`; three full `X / XGroup / XGroupSet` authoring triples with canonical DHIS2 naming — `organisation-units` / `organisation-unit-groups` / `organisation-unit-group-sets` (plus `organisation-unit-levels` for per-depth rename), `data-elements` / `data-element-groups` / `data-element-group-sets`, and `indicators` / `indicator-groups` / `indicator-group-sets`.

`dhis2 doctor` runs ~100 checks on a live instance (20 metadata-health probes + 81 DHIS2 integrity checks + BUGS tripwires).

### MCP surface

210 tools across 13 plugin groups (`analytics_*`, `apps_*`, `customize_*`, `data_*`, `dev_*`, `doctor_*`, `files_*`, `maintenance_*`, `messaging_*`, `metadata_*`, `profile_*`, `system_*`, `user_*`). Every CLI command has an MCP tool equivalent and vice versa; both share one typed service call.
...
- **Examples index** (`docs/examples.md`) catalogues 131 runnable examples (65 client, 41 CLI, 25 MCP) with descriptions + cross-links to concept docs.
...
The organisation-unit PR (#174) set a template — canonical DHIS2 resource names, hand-written accessors, per-item membership shortcuts, no `*Spec`. PRs #174 / #175 / #176 shipped the first three triples; the list below is what remains unauthored. Everything still reachable today via generic `dhis2 metadata list / get` + the generated CRUD accessors, but nothing more. Authoring them from the CLI or MCP means hand-crafting JSON and POSTing through `metadata import`:

- **Analytics triples still to ship**: `programIndicator` / `programIndicatorGroup` / `programIndicatorGroupSet`, `categoryOption` / `categoryOptionGroup` / `categoryOptionGroupSet`. Straightforward — same template as the three already shipped.
- **Category dimension** (the hardest corner): `Category`, `CategoryCombo`, `CategoryOptionCombo`. Tangled linkage + async regen of the CoC matrix. `categoryOption*` triples above are the shallow half; the `Category` / `CategoryCombo` stack is a strategic option on its own (see below).
...
Each of these blocks is a chunk of 1-3 PRs following the OU template. Near-term plan below picks up the remaining analytics triples + the tracker schema next.
...
Analytics-triples sweep is halfway done — org-units (#174), data-elements (#175), and indicators (#176) shipped. The list below keeps the pattern rolling and pairs it with the cheap-but-useful carryover items.

1. **`program-indicators` + `program-indicator-groups` + `program-indicator-group-sets` surface** — completes the analytics triples sweep. Program indicators embed an `expression` + `filter` (both DHIS2 expressions) and reference a `Program`; the accessor should expose `validate_expression(context="program-indicator")` the same way `client.indicators` does. Natural next step — third PR in the sweep, same template.
2. **`category-options` + `category-option-groups` + `category-option-group-sets` surface** — the fourth and last authoring triple. Straight triples pattern; does *not* include the Category → CategoryCombo → CategoryOptionCombo complexity (that's the [category-dimension authoring](#1-category-dimension-authoring) strategic option below). Landing just the triple here closes the triples chapter without committing to the full disaggregation surface.
3. **CI coverage gate** — wire `make coverage` into `.github/workflows/ci.yml` and upload `coverage.xml` as an artifact. `pytest-cov` + `coverage[toml]` are already dev deps; `[tool.coverage.run/report]` is configured. Pure workflow YAML change; carried over for the third cycle now. Small + cheap; pick up when someone has 20 minutes free.
4. **Bulk metadata patch on the client** — `client.metadata.patch_bulk(resource, [(uid, ops), ...])` runs a list of RFC 6902 patches in one request, returns `WebMessageResponse` with per-UID status. Complements `delete_bulk` + `save_bulk`. Motivated by the now-shipped triples: operations like "rename every DataElement whose code matches" and "rewrite legendSet on a cohort of visualizations" are the natural next CLI verbs to build, and they want a bulk-patch primitive underneath.
5. **Tracker schema authoring surface** — `Program` / `ProgramStage` / `ProgramStageDataElement` / `TrackedEntityType` / `TrackedEntityAttribute` all go through generic `metadata list/get` today. Pairs poorly with the already-shipped tracker *write* plugins (`dhis2 tracker register / enroll / add-event`): you can post tracked entities but can't create the program that receives them from the CLI. Promoted from medium-term now that the triples pattern is well established — tracker schema needs dedicated sub-apps per domain (own concepts: enrollment / incident date, repeating stages, PSDE ordering) rather than `XGroup/XGroupSet` triples.
...
- **Canonical `X / XGroup / XGroupSet` authoring triples** — three sub-apps each under `dhis2 metadata`, one client accessor per resource, following a single canonical-naming rule (lowercase + hyphenate the DHIS2 resource path). Shipped for `organisation-units` (#174), `data-elements` (#175), and `indicators` (#176). Each PR adds 15–18 MCP tools, full CLI verbs (`list` / `show` / `create` / rename-like / per-item membership), and hand-written accessors that return typed generated models. No `*Spec` builders — keyword args on the accessor (continues the spec-audit data point). The indicator accessor additionally exposes `validate_expression(context="indicator")` so callers can pre-flight numerator / denominator DE-UID references before a failed create. `program-indicators` and `category-options` triples are the remaining two to round out the analytics-authoring surface (see near-term).
