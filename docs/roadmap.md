...
3. **`JobConfiguration.jobParameters` codegen spec-patch** — same Jackson-discriminator gap as BUGS.md #14 (handled for `AuthScheme` in #76), applied to the 23-variant `JobParameters` polymorphic union. Requires the wire-tag → variant-class mapping from DHIS2's `JobType` enum + `@JsonSubTypes`. Once landed, `JobConfiguration.jobParameters` + `JobConfigurationParams.jobParameters` become typed discriminated unions. BUGS.md #15 tracks the upstream gap.
