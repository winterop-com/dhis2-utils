...
- **Core CRUD**: `list` / `get` / `patch` (RFC 6902) / `rename` (bulk name/shortName/description add + strip prefix/suffix, `--dry-run`) / `retag` (bulk ref-field + enum rewrites: categoryCombo, optionSet, legendSets, aggregationType, domainType).
...
313 tools across 13 plugin groups (`analytics_*`, `apps_*`, `customize_*`, `data_*`, `doctor_*`, `files_*`, `maintenance_*`, `messaging_*`, `metadata_*` (209), `profile_*`, `route_*`, `system_*`, `user_*`). Every CLI command has an MCP tool equivalent and vice versa; both share one typed service call.
...
- **Examples index** (`docs/examples.md`) catalogues 152 runnable examples (72 client, 49 CLI, 31 MCP) with descriptions + cross-links to concept docs. Tracker-schema authoring examples (steps 1 / 2 / 3 under `examples/cli/tracker_*.sh`) round-trip the full chain end-to-end.
...
Previous cycle wrapped cleanly — every item from the last refresh (#198) shipped: `metadata retag` verb (#199), `rename` strip-variants (#200), `list_members` sweep (#201), coverage threshold (#202). With tracker authoring complete end-to-end and both the `rename` + `retag` bulk verbs landed, the core DHIS2 workflow surface is now covered from the CLI side.

**The near-term slate is empty for the first time in months.** Every item that was queued as a known tactical cleanup has shipped. The natural next direction is either:
...
- **Roll medium-term items forward** for a lighter cycle while deciding between the three.

The list below leans on option two — four small / concrete items lifted from medium-term. Each is 1 PR; shipping them buys time to pick a strategic direction.

1. **`rootJunction=OR` client-side fallback** — BUGS.md #29: DHIS2 silently ANDs repeated `filter=` params even when `rootJunction=OR` is passed. `dhis2 metadata list --filter a --filter b --root-junction OR` still quietly ANDs on the server. Fix: emulate OR client-side by fanning out N requests (one per filter) and merging by UID, behind the same `--root-junction` flag. Gate with a `_dhis2_or_fallback` marker so the emulation drops cleanly when DHIS2 fixes the bug. Lands in `metadata list` + the matching accessor / MCP tool.
2. **`metadata sharing bulk`** — batched sharing helper. Apply one `SharingBuilder` result across many resources in one `/api/sharing` call per type. Natural follow-on to the bulk verbs shipped in #195 + #199. Small surface — `client.metadata.apply_sharing_bulk(by_resource, sharing)` + CLI wrapper. Useful when rolling a single user-group-access pattern across a cohort.
3. **Multi-version CI integration matrix** — slow-marked tests run against v42 only; committed codegen for v40 / v41 / v43 / v44 is never exercised against a live stack. Stand up compose-managed stacks in the nightly workflow (`e2e.yml`) and run the slow suite against each. Catches codegen drift before it reaches users. Pure YAML + docker-compose work; no Python changes.
4. **Spec-class audit decision** — the hand-written accessor tree has quietly drifted away from `*Spec` builders in favour of plain kwargs on `create(...)` (every triple + tracker accessor ships this way). Three older accessors still ship `*Spec` builders: `VisualizationSpec`, `MapSpec + MapLayerSpec`, `LegendSetSpec + LegendSpec`. Decision time: keep them (document the split in `api/visualizations.md` / `api/maps.md` / `api/legend-sets.md`) or deprecate + remove (breaking change; one PR). Revisit once someone's run into the spec-vs-kwargs friction in practice.
5. **Codegen determinism fix** — `dhis2 dev codegen oas-rebuild --version v42` produces ~1100 file diffs on every run against an unchanged `openapi.json`. Pattern is unused imports being added (`from typing import TYPE_CHECKING`, `from pydantic import Field as _Field`) to files that don't reference them, so the emitter is conditionally injecting imports based on traversal order or some non-stable predicate. Today every codegen run leaves a noisy working tree that has to be reverted by hand (or accidentally committed). Fix: find the non-deterministic predicate in `dhis2_codegen.oas` (likely set ordering or "did anything in this pass need `_Field`?" leaking across files) + add a regression test that runs the emitter twice against the same spec and asserts byte-equal output. Land alongside CI to catch future drift.
