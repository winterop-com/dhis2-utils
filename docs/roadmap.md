...
3. **Reshape `dhis2 doctor` — metadata-health, not bug-tripwire** — v1 ships 8 probes verifying that documented BUGS.md workarounds still apply. Useful for workspace maintenance (drift detection when DHIS2 fixes something upstream), but not what an operator actually asks "doctor" for. The next iteration should pivot the command toward diagnosing the **instance's own metadata health**: data sets with 0 data elements, data elements not attached to any dataset, indicators with broken expressions, org-unit-group memberships that don't cover every OU, user roles with no members, programs without stages, category combos with zero category options, etc. These are the classes of "this instance is misconfigured in a way DHIS2 won't flag at startup" surprises that cost operators real time. Concretely: (a) add a new `--mode metadata|bugs` flag (or split into `dhis2 doctor metadata` vs `dhis2 doctor bugs` sub-commands), (b) integrate DHIS2's own `/api/dataIntegrity/summary` + `/details` endpoints as first-class probes — the `maintenance dataintegrity` plugin already wraps them, doctor should compose on top — so users get DHIS2's built-in ~40 checks + our workspace-specific metadata sanity probes in one pass, (c) each metadata probe returns the list of offending UIDs so operators can jump straight to fixing them. Keep the bug/requirement probes reachable but stop making them the primary output.
