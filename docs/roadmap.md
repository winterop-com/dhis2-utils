...
3. **Data-integrity streaming iterator** — `client.maintenance.iter_integrity_issues(...)` yielding one issue at a time for large integrity runs. Small addition to the maintenance plugin; wraps the existing `/api/dataIntegrity/details` pagination.
...
- ~~**Library-level task awaiter**~~ — shipped. `client.tasks.await_completion(task_ref, ...)` + `client.tasks.iter_notifications(...)` for streaming renderers. Typed `TaskCompletion` result, `TaskTimeoutError` on timeout.
