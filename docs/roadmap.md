...
- **Core CRUD**: `list` / `get` / `patch` (RFC 6902) / `rename` (bulk name/shortName/description rewrite with `--dry-run`).
...
312 tools across 13 plugin groups (`analytics_*`, `apps_*`, `customize_*`, `data_*`, `doctor_*`, `files_*`, `maintenance_*`, `messaging_*`, `metadata_*` (208), `profile_*`, `route_*`, `system_*`, `user_*`). Every CLI command has an MCP tool equivalent and vice versa; both share one typed service call.
...
- Auto-generated **CLI reference** (`docs/cli-reference.md`, ~3700 lines from the Typer app) + **MCP reference** (`docs/mcp-reference.md`, 312 tools across 13 groups from the FastMCP server). Both regenerated on every `make docs-build`.
...
- **Examples index** (`docs/examples.md`) catalogues 152 runnable examples (72 client, 49 CLI, 31 MCP) with descriptions + cross-links to concept docs.
...
799 tests run via `make test` (830 collected including 31 slow-marked for the nightly integration stack). Unit + CliRunner + respx-mocked HTTP. Slow tests exercise live-stack workflows (`--watch` job polling, Playwright PAT creation, dashboard screenshot capture, Playwright-driven OIDC login). `make coverage` runs branch-coverage locally + on every CI run (produces `coverage.xml` as an artifact); no failure threshold yet.
...
Previous cycle wrapped cleanly — every item from the last refresh shipped: tracker-schema step 3 (#194), `dhis2 metadata rename` verb (#195), CI coverage gate (#196), list-path `*_raw` sweep (#197). With tracker authoring complete end-to-end + the bulk-patch primitive surfaced as a CLI verb, the remaining slate is smaller and more tactical.

1. **`dhis2 metadata retag` verb** — natural follow-on to `metadata rename` (#195). Rewrite a ref field (`legendSet`, `optionSet`, `categoryCombo`, `aggregationType`, …) across a filtered cohort. Same scaffolding + `patch_bulk` primitive underneath, different op shape: `replace /legendSet/0/id` vs `replace /name`. One focused PR.
2. **Strip-prefix / strip-suffix on `metadata rename`** — the existing verb is add-only. `--strip-name-prefix "ANC: "` removes the prefix where present (idempotent: no-op when missing). Reverses the direction cleanly; avoids callers reaching for raw `metadata patch` to undo a bulk prefix. Small extension.
3. **`list_members` accessor sweep** — closes the final remaining raw-list pattern. Group accessors query the _member_ resource with a parent-scope filter (e.g. `/api/indicators?filter=indicatorGroups.id:eq:<uid>`). Today those use `get_raw` + comprehension; could map to `resources.indicators.list(filters=[f"indicatorGroups.id:eq:{uid}"])`. Couples the group accessor to a different resource attr but harmonises with the rest of #197. Maybe 10 files.
4. **Coverage failure threshold** — follow-up to #196. `make coverage` writes `coverage.xml` but doesn't fail under any threshold. Add `coverage report --fail-under=70` (pick a number that the current suite comfortably clears — 74% today — so floor feels real without forcing churn). Tiny Makefile + CI tweak.
...
- **`dhis2 metadata rename` verb** — bulk-rename CLI on top of `client.metadata.patch_bulk` (#195). `--filter` (repeatable, same DSL as `metadata list`) + `--name-prefix` / `--name-suffix` / `--short-name-prefix` / `--short-name-suffix` / `--set-description`. Mutations are idempotent (won't double-prefix already-prefixed values). `--dry-run` renders a Rich before/after table without patching. Per-UID failures land in the shared `ConflictRow` renderer used by `metadata import`, so operators see row-level detail on partial failures.
- **CI coverage gate** (#196). `make coverage` replaces `make test` in the CI test step; every run uploads `coverage.xml` as an artifact retained 14 days. Local baseline: 74.35% across 84k statements / 6.7k branches. No failure threshold yet — next near-term item lands that.
