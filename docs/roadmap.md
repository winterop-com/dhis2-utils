...
3. **Sweep remaining raw-client callsites in examples + diagnostic tooling** — the service-layer `get_raw`/`post_raw` calls are all wrapped in `Model.model_validate(raw)` on the next line (Bucket B carveout), but ~30 `examples/client/*` and `infra/scripts/*` raw calls could be upgraded to typed accessors on a case-by-case basis. Low-urgency follow-up to the four-PR typing sweep (#71-#74) — docs / tooling beats the dry-rule on readability in examples, so this is judgement rather than policy.
4. **`JobConfiguration.jobParameters` codegen spec-patch** — same Jackson-discriminator gap as BUGS.md #14 (handled for `AuthScheme` in the codegen spec-patches sweep), applied to the 23-variant `JobParameters` polymorphic union. Requires the wire-tag → variant-class mapping from DHIS2's `JobType` enum + `@JsonSubTypes`. Once landed, `JobConfiguration.jobParameters` + `JobConfigurationParams.jobParameters` become typed discriminated unions and the scheduler plugin can drop its remaining `dict[str, Any]`. BUGS.md #15 tracks the upstream gap + downstream workaround options.
