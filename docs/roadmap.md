...
- Auto-generated **CLI reference** (`docs/cli-reference.md`, ~3700 lines from the Typer app) + **MCP reference** (`docs/mcp-reference.md`, 303 tools across 13 groups from the FastMCP server). Both regenerated on every `make docs-build`.
...
- **Examples index** (`docs/examples.md`) catalogues 151 runnable examples (72 client, 48 CLI, 31 MCP) with descriptions + cross-links to concept docs.
...
775 tests run via `make test` (806 collected including 31 slow-marked for the nightly integration stack). Unit + CliRunner + respx-mocked HTTP. Slow tests exercise live-stack workflows (`--watch` job polling, Playwright PAT creation, dashboard screenshot capture, Playwright-driven OIDC login). `make coverage` runs branch-coverage locally but CI doesn't gate on it yet.
...
The organisation-unit PR (#174) set a template — canonical DHIS2 resource names, hand-written accessors, per-item membership shortcuts, no `*Spec`. The triples sweep (#174 / #175 / #176 / #180 / #181), the aggregate data-set surface (#185), the validation-rule + predictor CRUD (#186), and tracker-schema steps 1 + 2 (#188 + #189) have all landed on top of it. Remaining unauthored pieces:

- **Tracker schema — inner layer**: `ProgramStage` + `programStageDataElements[]` + optional `ProgramStageSection`. The outer layer (`TrackedEntityType` / `TrackedEntityAttribute` / `Program` / PTEA / program-OU scope) ships; the inner layer — what DEs a stage collects, in what order, optionally grouped into sections — still means hand-rolled JSON.
...
Near-term plan below picks up tracker step 3 next.
...
The big sweeps from the previous cycle all landed — aggregate data-sets (#185), validation-rule + predictor CRUD (#186), bulk metadata patch (#187), tracker-schema steps 1 + 2 (#188 + #189), and the codegen + hand-written `*_raw` audit chain (#190 → #191 → #192, ~530 lines of duplication removed). The list below picks up the remaining gaps + the cheap-but-useful carryovers.

1. **Tracker-schema step 3 — `ProgramStage` + PSDE** — completes tracker authoring end-to-end. Each Program owns an ordered list of stages (ANC 1st visit, 2nd visit, …); each stage owns `programStageDataElements[]` (a join table with `compulsory`, `displayInReports`, `sortOrder`, `allowProvidedElsewhere` flags). Shape parallels DataSet + DataSetElement: hand-written accessor with `add_element` / `remove_element` / `reorder` helpers, plus optional `ProgramStageSection` for in-stage grouping. Probably 1–2 PRs. Unblocks the full tracker-schema-from-CLI story: author TET + TEAs + Program (shipped), then define which DEs its stages collect (this PR).
2. **Bulk metadata CLI verbs on top of `patch_bulk`** — the primitive shipped in #187 has no direct CLI exposure yet. Worth turning into concrete operator-level commands: `dhis2 metadata rename --resource dataElements --filter "code:like:DE_ANC" --prefix "ANC: "` to bulk-rename, `dhis2 metadata retag --resource visualizations --filter ... --legend-set <uid>` to rewrite a ref across a cohort. Leans directly on `BulkPatchResult` + `ConflictRow` rendering already shipped for `metadata import`. One focused PR.
3. **List-path `*_raw` sweep (follow-up to #191 + #192)** — paged reads (`list_all` / `list_members` / `list_children`) still use `get_raw` + a comprehension of `model_validate`. Many cases map cleanly onto `resources.X.list(fields=..., filters=[...], paging=False)` — the codegen template already supports repeatable filters + orders. Mechanical, case-by-case (some list methods mix custom filters + resource-specific selectors that don't fit the generated list shape), but the rest drop ~200 more lines of duplication.
4. **CI coverage gate** — wire `make coverage` into `.github/workflows/ci.yml` and upload `coverage.xml` as an artifact. `pytest-cov` + `coverage[toml]` are already dev deps; `[tool.coverage.run/report]` is configured. Pure workflow YAML change; carried over for the fifth cycle now. Small + cheap; pick up when someone has 20 minutes free.
...
- **Tracker-schema authoring (steps 1 + 2)** — `dhis2 metadata tracked-entity-attributes` + `tracked-entity-types` (#188) covers the leaf resources; `tracked-entity-types add-attribute --mandatory --searchable` round-trips the TETA join table. `dhis2 metadata programs {list, show, create, rename, add-attribute, remove-attribute, add-to-ou, remove-from-ou, delete}` (#189) covers the middle layer — WITH_REGISTRATION / WITHOUT_REGISTRATION program flavours, PTEA enrollment form linkage, per-item OU shortcuts. Documents the DHIS2 `mergeMode=REPLACE` requirement on Program PUT (nested-list removal is additive without it) as a typed client-side workaround. Step 3 (ProgramStage + PSDE) on the near-term list.
- **Codegen + base-client gap closure** (#190–#192). Generated `create(item, *, merge_mode, import_strategy, skip_sharing, skip_translation)` + `update(item, ...)` forward the write-flag query params. Every generated resource exposes `add_collection_item(parent_uid, collection, item_uid)` / `remove_collection_item(...)` for per-item POST/DELETE shortcuts. Base `Dhis2Client` ships typed `post(path, body, model=T)` + `put(path, body, model=T)` wrappers (parallels the existing typed `get`). Hand-written accessor sweep across 28+24 files replaced `_put_with_replace` / per-item loops / duplicated `_uid_from_webmessage` helpers / single-object `get_raw + model_validate` with the new surface. ~530 lines of duplication removed with no behavior change.
