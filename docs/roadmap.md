...
1. **`MetadataBundle` type — push generated pydantic models all the way through the metadata plugin.** Today `export_metadata` returns `dict[str, Any]` (DHIS2's raw `GET /api/metadata` body), and `list_metadata` / `get_metadata` parse generated models (`DataElement`, `Indicator`, ...) only to dump them back to `dict[str, Any]` for MCP serialisation. That violates the "no `dict[str, Any]` outside the HTTP boundary" rule — the bundle flows through diff / import / dangling-ref walker without ever being wrapped. Fix: introduce `MetadataBundle(BaseModel)` with typed `system` / `date` slots and an `extra="allow"` map of resource-name → `list[<TypedModel>]`; change `list_metadata` / `get_metadata` to return the typed generated models; push `_dump()` down to the MCP edge (the only layer that genuinely needs a JSON-dict shape). Touches every bundle-consuming callsite — own PR, near-zero behaviour change, pure plumbing.
