...
1. **`metadata sharing bulk`** — batched sharing helper. Apply one `SharingBuilder` result across many resources in one `/api/sharing` call per type. Natural follow-on to the bulk verbs shipped in #195 + #199. Small surface — `client.metadata.apply_sharing_bulk(by_resource, sharing)` + CLI wrapper. Useful when rolling a single user-group-access pattern across a cohort.
2. **Multi-version CI integration matrix** — slow-marked tests run against v42 only; committed codegen for v40 / v41 / v43 / v44 is never exercised against a live stack. Stand up compose-managed stacks in the nightly workflow (`e2e.yml`) and run the slow suite against each. Catches codegen drift before it reaches users. Pure YAML + docker-compose work; no Python changes.
3. **Spec-class audit decision** — the hand-written accessor tree has quietly drifted away from `*Spec` builders in favour of plain kwargs on `create(...)` (every triple + tracker accessor ships this way). Three older accessors still ship `*Spec` builders: `VisualizationSpec`, `MapSpec + MapLayerSpec`, `LegendSetSpec + LegendSpec`. Decision time: keep them (document the split in `api/visualizations.md` / `api/maps.md` / `api/legend-sets.md`) or deprecate + remove (breaking change; one PR). Revisit once someone's run into the spec-vs-kwargs friction in practice.
