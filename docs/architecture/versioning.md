...
- `schemas_manifest.json` — snapshot of the `/api/schemas` response used at generation time. Committed so `dhis2 codegen rebuild` can regenerate offline.
...
3. Runs `dhis2 codegen generate` against `http://localhost:8080` with admin/district, which writes `generated/v{N}/schemas/`, `resources.py`, `__init__.py`, and `schemas_manifest.json`.
...
```bash
uv run dhis2 codegen rebuild                              # every v{N}/schemas_manifest.json
uv run dhis2 codegen rebuild --manifest path/to/foo.json  # just one
```
