...
1. Brings up a fresh `dhis2/core:N` stack with an empty-gzip placeholder where `infra/dhis-v42.sql.gz` sits (so Flyway bootstraps a clean schema instead of trying to load a v42 dump into a v40 stack).
